...
class Env:
    def __init__(self,is_senior=False,seed=100, gui=False):
        ...
        self.observation_space = gym.spaces.Box(low=-1, high=1, shape=(12,), dtype=np.float64)
    ...
    def step(self, action):
        ...
        return (self.get_observation(), self._get_reward(), self.terminated, self.terminated, self._get_info())
        # return self.observation
    ...
    def reward(self):
        ...
        if self.get_dis() < 0.05 and self.step_num <= self.max_steps:
            ...
            self.terminated = True
            print("Terminated for reaching max steps, rewoard: ", self._get_reward())
    ...
    def _get_reward(self):
        ...
        return 100*(- self.get_dis() - self.obstacle_contact * 0.3 + self.success_reward / 100
            - np.sum(a - np.array([a_t_n, a_t_n3, 0.40097904, 0.04461199 - a_t_n3 + 0.07, a_t_n2, 0.5])))
